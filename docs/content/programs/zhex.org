...
=zhex= prints a color-coded hex dump of files or stdin, using the same output format and color scheme as [[https://github.com/sharkdp/hexyl][hexyl]].
...
#+begin_example
$ echo "Hello, World!" | zhex --no-color
┌────────┬─────────────────────────┬─────────────────────────┬────────┬────────┐
...
#+begin_src bash
zhex -s 16 -n 128 file.bin
#+end_src

*** Pipe from another command
#+begin_src bash
curl -s https://example.com | zhex | head -20
#+end_src
